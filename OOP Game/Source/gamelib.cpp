...
//Precondtion:
//Postcondition: Returns the index of the current CMovingBitamp displayed. 	
{
	return bmp_counter;
}

int CAnimation::Height()
//Precondition:	CAnimation object CMovingBitmap list (bmp) must be nonempty
//Postcondition: Returns height of CMovingBitmap pointed to by the CMovingBitmap 
//list iterator
{
...
int CAnimation::Left()
//Precondition: CAnimation object CMovingBitmap list (bmp) must be nonempty.
//Postcondition: Returns the x-axis coordinate of the top left corner of the object.
{
	GAME_ASSERT(bmp.size() != 0,"CAnimation: Bitmaps must be loaded first.");
	return x;
}
...
void CGameState::ShowInitProgress(int percent)
{
	if (!SHOW_LOAD_PROGRESS)
		return;
	//SIZE_X 640 and SIZE_Y 480 (Macros for Default Screen Dimensions)
	const int bar_width = SIZE_X * 2 / 3;	
	const int bar_height = SIZE_Y / 20;
	const int x1 = (SIZE_X - bar_width) / 2;	// Centered Bar Left-Edge Value
	const int x2 = x1 + bar_width;	// Centered Bar Right-Edge Value
	const int y1 = (SIZE_Y - bar_height) / 2;	// Centered Bar Top-Edge Value
	const int y2 = y1 + bar_height;	// Centered Bar Bottom-Edge Value
	const int pen_width = bar_height / 8;	// Border Thickness
	const int progress_x1 = x1 + pen_width;	// Percent Loading Bar Min-Edge
	const int progress_x2 = progress_x1 + percent * (bar_width-2*pen_width) / 100;	// Percent Loading Bar Right-Edge
	const int progress_x2_end = x2 - pen_width;	// Percent Loading Bar Max-Edge
	const int progress_y1 = y1 + pen_width;	// Percent Loading Bar Top-Edge
	const int progress_y2 = y2 - pen_width;	// Percent Loading Bar Bottom-Edge

	CDDraw::BltBackColor(DEFAULT_BG_COLOR);		// Background Plain
	CMovingBitmap loading;						// CMovingBitmap loading
